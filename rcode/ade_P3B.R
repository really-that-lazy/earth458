...
u = (((k_*0.4))/n_)/R	   #Velocity Left to Right (m/s) 
cat('\n\n u ', u, '\n')
disp = ( ((8.2*10^-10)*0.8) + (u)*(0.02))/R      #Dispersion constant (m^2/s)
cat('\n\n disp ', disp, '\n')
...
pdf(file="ADE_CDprofile_a4q3b.pdf")
par(mar=c(6,6,2,2))
...
pdf(file="ADE_BTcurve_a4q3b.pdf")
par(mar=c(6,6,2,2))
plot(xrange, yrange, type="n", ylab = " ", xlab = " ", main = " ", cex.axis = 2)
