...
L = 200		   #Length of Problem (m) Make it a little longer than the point of interest
dz = ceiling(L/10)/10  #Discretization (m)
R = 1.             #Retardation Factor
disp = 2.0*10^-7/R      #Dispersion constant (m^2/s)
u = 3.5*10^-6/R	   #Velocity Left to Right (m/s) 
cat('disp ', disp)
cat('\nu ', u, '\n')
...
LeftHandC = 0   #Left hand boundary concentration (only for constant concentration conditions)
...
StartOfPlume = 25 #This is the start of the Plume (m) 
EndOfPlume = 50    #This is the ending value of the Plume (m) 
PlumeC = 25       #Concentration of Plume (mg/L)
...
pdf(file="ADE_CDprofile_a4q1.pdf")
par(mar=c(6,6,2,2))
...
graphics.off()
cat('calcing xy ranges\n')
xrange <- range(0,Runtime*1.2)
yrange <- range(0,(max(C[,1])*1.25))
cat('xrange ', xrange)
cat('\nyrange ', yrange, '\n\n')
cat('finished calcing xy ranges')
pdf(file="ADE_BTcurve_a4q1.pdf")
par(mar=c(6,6,2,2))
plot(xrange, yrange, type="n", ylab = " ", xlab = " ", main = " ", cex.axis = 2)
